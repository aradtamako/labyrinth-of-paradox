...
- **インタラクティブマップ**（1〜35区域）— マップをノードのグリッドとして描画し、マスにカーソルを
  乗せるとノード種別・等級・報酬が出る。クリックで固定表示。
- **1〜35区域のシードマップ画像** — 元記事の画像を掲載。シードごとの判別にも使える。
- **シードコード検索** — ゲーム内でマップ右端の列の部屋数を上から数えた数字（例 `23333`）から、
  該当するシードを全区域横断で検索できる。
- **報酬一覧・検索** — どのマスでどの報酬が出るかを逆引きできる。報酬名・ノード種別・等級で検索。
- **名声上昇値・主要報酬** — 区域ごとの推奨名声の推移と主な報酬。
- **区域メモ** — 区域カードからそのブラウザにだけ残せる自分用の攻略メモ。メモ本文で検索したり、
  メモの有無で一覧を絞れたりする。他人には見えない。
- **システム解説** — 迷宮調査券／入場券、討伐券7種、傭兵団、バッファー派遣の仕様。
- **更新履歴** — サイト側で変わったこと（区域の収録、表示、検索の挙動）をタイムラインで。該当 PR へ
  飛べる。ゲーム内の仕様変更そのものは扱わない。
...
| [dnf.umi.cat](https://dnf.umi.cat/kr) | マスごとのノード配置・種別・等級・報酬（1〜35区域を取り込み済み） |
