...
- **インタラクティブマップ**（1〜33区域）— マップをノードのグリッドとして描画し、マスにカーソルを
  乗せるとノード種別・等級・報酬が出る。クリックで固定表示。
- **1〜33区域のシードマップ画像** — 元記事の画像を掲載。シードごとの判別にも使える。
...
| [dnf.umi.cat](https://dnf.umi.cat/kr) | マスごとのノード配置・種別・等級・報酬（1〜33区域を取り込み済み） |
