...
import { thumbFor } from '@/data/floors'
    ...
    <>
      <figure className="group flex flex-col overflow-hidden rounded-xl border bg-card shadow-sm transition-shadow hover:shadow-md">
        <button
          ...
          aria-label={`${floorLabel} ${image.seed ?? `マップ ${index + 1}`} を拡大表示`}
        >
          <img
            src={thumbFor(image.src)}
            alt={`${floorLabel} ${image.seed ?? `マップ ${index + 1}`}`}
            loading="lazy"
            decoding="async"
        ...
        {(image.seed || image.caption) && (
          <figcaption className="flex flex-col gap-1.5 border-t px-3 py-2.5">
            ...
            {image.caption && (
              <span className="text-xs leading-relaxed text-muted-foreground">{image.caption}</span>
            )}
          </figcaption>
        )}
      </figure>
      ...
      <MapLightbox
        ...
        image={image}
        title={`${floorLabel}｜${image.seed ?? `マップ ${index + 1}`}`}
      />
    </>
  )
}
